...
elementAtRecursive xs total_len idx = if total_len - myLength (tail xs) == idx
                                      then
                                        [head xs]
                                      else
                                        elementAtRecursive (tail xs) total_len idx
...
-- #5
myReverse xs = if myLength xs == 1
               then
                    xs
               else
                    myLast xs : myReverse (myInit xs)
